...
class TranGroupsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create tran_group' do
    assert_difference('TranGroup.count') do
      post tran_groups_url,
           params: { tran_group: { amount: @tran_group.amount, groupName: @tran_group.groupName, icon: @tran_group.icon,
                                   user_id: @tran_group.user_id } }
    end
  ...
  test 'should update tran_group' do
    patch tran_group_url(@tran_group),
          params: { tran_group: { amount: @tran_group.amount, groupName: @tran_group.groupName, icon: @tran_group.icon,
                                  user_id: @tran_group.user_id } }
